        ...
        return axios(requestOptions).then((response) => {
            ...
            if (!options.customer) {
                if (data.status === 1) {
                    ...
                    // e.name = data.code == null ? e.name : data.code;
                    // throw e;
                    vm.$super.toast(data.message || data.code)
                }
            } else { //七牛请求数据
                return data;
            }
...
const LOGIN_IP = 'http://120.55.151.61:80'; //测试外网http://121.40.28.189:81 内网http://192.168.0.8:80 正式外网http://120.55.151.61:80
let API = {
    // sources: sources,
